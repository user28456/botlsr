...
if(message.author.id != "493003491560783872") return message.channel.send(bakim)
  ...
  if (message.author.id !== message.guild.owner.user.id)
    return message.reply(
      "<a:attention:836376259960897557> Bu komutu kullanabilmek için **Sunucu Sahibi** olmalısın!"
    );
  let prefix = "kb!";
  let kanal = message.mentions.channels.first();
  if (!kanal) {
    message.channel.send(
      `<a:781157570785443851:833983185377296384> Bir kanal etiketlemelisin!`
    );
    return;
  }
  db.set(`westralogkanal_${message.guild.id}`, kanal.id);
  message.channel.send(
    ` Başvuru log kanalı başarıyla ${kanal} olarak ayarlandı!`
  );
};
